...
> **문서 수 안내.** 문제지는 `products` 20,000건을 전제하지만 배포된 데이터 파일이
> `products-10000.ndjson`이라 클러스터에는 **10,000건**이 들어 있다. 강사님께 확인한
> 결과 건수는 상관없다고 하셔서, 교재 숫자를 베끼지 않고 내 클러스터에서 확인한 값을
> 그대로 적었다.
...
- 정상/보류/오류: **정상**
- 판정 근거: Data View·time field·field 구성이 전부 맞고, Discover 건수도
  `GET /products/_count`(10,000)와 일치한다. 교재는 20,000 기준이지만 배포 데이터가
  10,000건이고 강사님이 건수는 상관없다고 하셔서 정상으로 판정했다.
...
**GREEN.** 필수 1~4 완료. 마지막 상태는 KQL·filter 없이 전체 10,000건이고,
Data View·time field·7개 field·복구를 모두 확인했다. 교재의 20,000과 다른 것은 배포
데이터 차이이며 강사님 확인을 받았다.
