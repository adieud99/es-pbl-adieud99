...
- 캡처 파일: 별도 캡처 없음. `_count` 비교로 대신했다.
...
- 수정 후 문서 수: **10,000**
- 다음부터 먼저 확인할 항목: **시간 범위**를 제일 먼저 본다. 0건이 나오면 index 문제부터
  의심하기 쉬운데, 실제로는 시간 범위가 원인인 경우가 대부분이다. `_count`로 index 전체
  건수를 먼저 확인하면 index가 살아 있는지 1초 만에 갈린다.
- 캡처 파일: 별도 캡처 없음.
