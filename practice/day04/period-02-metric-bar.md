...
`products` 전체는 **10,000**건이다. 교재는 20,000 기준이지만 배포 데이터 차이이며
강사님 확인을 받았다(1교시 참고).
...
- 정상/보류/오류와 이유: **정상.** 계산 방식·시간 범위·조건이 모두 맞고 값이
  `GET /products/_count`(10,000)와 일치한다.
...
**GREEN.** Metric 10,000, category Bar 8개, 제목 2개, 방향 비교와 복구 기록을 모두
마쳤다. 8 × 1,250 = 10,000으로 Metric과 합계가 맞는 것까지 확인했다.
